...
`;
    ...
    parseText(item, xsdItem, sender) 
    {
        ...
        if (item.attr.element) {
            ...
            itemStart = sender.parseAttrs(itemStart, item, xsdItem, sender);
    ...
    parseAttrs(itemStr, item, xsdItem, sender) {

        let htmlBase = sender.HTMLObjects[sender.HTMLObjects.length -1];
        let groupBase = htmlBase.itemObject.groups[htmlBase.itemObject.groups.length -1]

        let parent = groupBase.itemObject.xsdXML.childWithAttribute("name", item.attr.element);

        console.log("parent", xsdItem);
 
        itemStr = itemStr.replace("${ew-map}", parent.name + "::" + item.attr.element);
