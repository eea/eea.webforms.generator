    ...
    createHTMLOutput() 
    {
        ...
        for (let f = 0, t = this.htmlOutput.HTMLObjects.length; f < t; f++) {
            ...
            for (let i = 0, l = groups.length; i < l; i++) {
                ...
                formHtml.push(Array(i + 2).join("\t") + groups[i].itemObject.tagstart);

                // console.log("groups.itemObject.items.length", groups[i].itemObject.items.length);
                formHtml.splice(groups[i].itemObject.items.length + 1, 0, Array(i + 2).join("\t")  + groups[i].itemObject.tagend);
                // console.log("formHtml", formHtml);

                for (let i2 = 0, l2 = groups[i].itemObject.items.length; i2 < l2; i2++) {
                    formHtml.splice( ((i + 1) * i2) + 1, 0, Array((i + 1) * 2).join("\t\t") + groups[i].itemObject.items[i2]);
                }
