...
class XSDWebFormParser 
{
    ...
    createHTMLOutput() 
    {
        ...
        for (let f = 0, t = this.htmlOutput.HTMLObjects.length; f < t; f++) {
            ...
            html.splice(html.length + 1, 0, this.htmlTagParser.HTML_TYPES[form.type].htmlTemplate);

            let groups = this.htmlOutput.HTMLObjects[f].itemObject.groups;
            // console.log("groups", groups);

            html.splice(groups.length + html.length + 1, 0, this.htmlTagParser.HTML_TYPES[form.type].htmlTemplateE);
            ...
            for (let i = 0, l = groups.length; i < l; i++) {
                ...
                formHtml.push(Array(i + 2).join("\t") + this.htmlTagParser.HTML_TYPES[group.type].htmlTemplate);

                formHtml.splice(groupObjHTML.children.length + 1, 0, Array(i + 2).join("\t") + this.htmlTagParser.HTML_TYPES[group.type].htmlTemplateE);
                // console.log("formHtml", formHtml);

                for (let i2 = 0, l2 = groupObjHTML.children.length; i2 < l2; i2++) {
                    formHtml.splice( ((i + 1) * i2) + 1, 0, Array((i + 1) * 2).join("\t\t") + groupObjHTML.children[i2]);
                }
