...
class XSDWebFormParser 
{
    ...
    xsdInnerParse(xmltItem) 
    {
        ...
        console.log(`${xspace}L:${xmltItem.level}\n`);

        if (xmltItem.children)
            console.log(` ${xspace}|||||||||||| ${xmltItem.name} :: ${xmltItem.attr.name}`);
            // console.log(`\n[ =====================>---------------- ${xmltItem.name} | ${xmltItem.type} | ${xmltItem.children.length} ]`);

            if (xmltItem.xparent) {
                console.log(`${xspace}|||||||||||| parent :: ${xmltItem.xparent.name}`);

            if (xmltItem.xparent.attr.name) {
                console.log(`${xspace}|||||||||||| par.name :: ${xmltItem.xparent.attr.name}`);
            }
        }
