...
class XSDWebFormParser 
{
    ...
    xsdParse(xml) 
    {
        ...
        var hmlt = new XmlDocument(xml.hdata);
        hmlt.level = 0;
        this.xmlInnerParse(hmlt);
    ...
    /**
     * xmlInnerParse - Parse inner XML Document
     * @param xmltItem
     */
    xmlInnerParse(xmltItem) 
    {console.log( "-----------" + xmltItem );
        // var formNode = xmltItem.childrenNamed("form")[0]; 
       
        // Loop through Tag's childNodes
        // if (formNode) {
                for (let i = 0, l = xmltItem.children.length; i < l; i++) {
                    if (xmltItem.children[i].type === "element") {
                        console.log( "-----------" + xmltItem.children[i].name );
                        this.xmlInnerParse(xmltItem.children[i]); 
                    }
            };
        // }
